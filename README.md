...
<h3>FasterEdge 框架的 PY32F003（Cortex-M0+） 平台实现（Keil MDK / PlatformIO 版）</h3>
</div>

### 一、简介

本项目是 **[FasterEdge](https://github.com/FasterEdge/FasterEdge)** 框架在 ****PY32F003（Cortex-M0+）****平台上的实现。PY32F003 为 32 位 Cortex-M0+ 内核：24MHz、16KB Flash、2KB RAM、无硬件 EEPROM，无网络、无操作系统，因此按 [MCU-C51](../MCU-C51) 的无网络精简思路裁剪能力子集，并保留 **寄存器 / GPIO / 芯片信息** 三个 MCU 专有模块。
...
| 能力 | 排除原因 |
|------|---------|
| MQTTAbility / NetMapData | PY32F003 无网络协议栈 |
...
└── platformio_ide/             # VS Code + PlatformIO IDE 工程（py32 平台 + ARM GCC）
...
> 两套均为裸机 C 工具链：`keil/`（Keil MDK 官方编译器）与 `platformio_ide/`（ARM GCC，VS Code 插件），能力与命令完全一致。

### 五、使用说明

1. **keil 版**：用 Keil MDK 打开 `keil/MDK-ARM/FasterEdge-MCU-PY32F003.uvproj`（或按芯片新建 ARM 工程），编译后 SWD 烧录
2. **platformio_ide 版**：VS Code 安装 **PlatformIO IDE** 插件，打开 `platformio_ide/`，Build / Upload / Serial Monitor
3. 在 `fe_port.c` 中按你的 PY32 型号调整 UART/GPIO 引脚（已附寄存器参考）
...
`platformio_ide/` 为 **ARM GCC 版** 工程（PlatformIO py32 平台），`fe_port.c` 为 Cortex-M0+ 寄存器级参考实现：

| 功能 | 实现 |
|------|------|
| UART | **USART1**（RCC + BRR + CTRL/STATR/DATAR）|
| EEPROM | Flash 模拟参考（TODO 已留接口）|
| 时间 | **SysTick**（1ms）|
| GPIO | **GPIOA/B/C**（MODER/IDR/ODR/BSRR 风格寄存器）|
| 随机数 | LCG |
...
除主仓库对应能力外，本仓库提供 3 个 **MCU 专有** 模块（寄存器 / GPIO / 芯片信息）。PY32F003 的寄存器为 **32 位 Cortex-M0+ 外设空间**（0x40000000+），GPIO 为引脚号 0-19：
...
| ChipData | Data | `info` | PY32F003 型号 / RAM / Flash / 频率 |

**示例：**

```
ability_RegAbility read 0x40013800      # 读 USART1 STATR
ability_RegAbility write 0x4001380C,0x0C # 写 USART1 CTRL1
